...
contract YIeldToken is ERC20('PolyYIeld Token', 'YIELD') {
...
// MasterChef is the master of Yield. He can make Yield and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Yield is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accYieldPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
    ...
    uint256 public YieldPerBlock = 0.015 ether;
    ...
    // Yield referral contract address.
    ...
    // View function to see pending Yieldes on frontend.
    function pendingYield(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accYieldPerShare = accYieldPerShare.add(YieldReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accYieldPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 YieldReward = multiplier.mul(YieldPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Yield.mint(devAddress, YieldReward.div(10));
        Yield.mint(address(this), YieldReward);
        pool.accYieldPerShare = pool.accYieldPerShare.add(YieldReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accYieldPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accYieldPerShare).div(1e18);
    ...
    // Safe Yield transfer function, just in case if rounding error causes pool to not have enough FOXs.
    ...
    function updateEmissionRate(uint256 _YieldPerBlock) external onlyOwner {
        massUpdatePools();
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                Yield.mint(referrer, commissionAmount);
